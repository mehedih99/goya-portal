(() => {
  ...
  async function api(action, payload={}) {
    const base = String(state.apiUrl || C.API_URL || "").trim();
    if (!base) throw new Error("Google Apps Script Web App URL is not configured.");

    const callbackName = "__goya_cb_" + uid();
    const params = new URLSearchParams();
    params.set("action", action);
    const requestPayload = Object.assign({}, payload || {});
    if (state.token && !requestPayload.token) requestPayload.token = state.token;
    ...
    return new Promise((resolve, reject) => {
      ...
      let finished = false;
      const timeout = setTimeout(() => finishReject("Connection timed out. Check the Apps Script deployment and access setting."), 20000);
      ...
      function finishResolve(data) {
        ...
        if (data && data.ok === false) {
          finishReject(data.error || "Server error.");
          return;
        }
        finished = true;
        cleanup();
      ...
      script.onerror = () => finishReject("Could not connect to Google Apps Script. Make sure the Web App is deployed as Anyone and the /exec URL is correct.");
  ...
  function renderLogin(){
    document.getElementById("app").innerHTML = `<div class="pin-screen">
      <div class="auth-card">
        ...
        <p class="note" style="margin-top:18px">Backend: ${state.apiUrl ? "Google Apps Script connected" : "Not configured"}</p>
      </div>
    </div>`;
  ...
  async function staffLogin(){
    ...
    const renderAuth=()=>{
      ...
      document.querySelectorAll(".key").forEach(b=>b.onclick=async()=>{ const k=b.dataset.key; if(k==="⌫") pin=pin.slice(0,-1); else if(k==="Enter"){ if(!selected){toast("Select a staff member.");return;} if(pin.length!==4){toast("Enter 4 digits.");return;} try{const result=await api("loginStaff",{name:selected["Staff Full Name"],pin}); if(result && result.ok && result.token){state.token=result.token;state.role="staff";state.staff=result.staff;state.admin=false;state.tab="order";await loadData();render();}else{toast("Invalid PIN.");pin="";renderAuth();}}catch(e){toast(e.message);}} else if(pin.length<4) pin+=k; renderAuth(); });
  ...
  async function adminLogin(){
    ...
    const renderAuth=()=>{ document.getElementById("app").innerHTML=`<div class="pin-screen"><div class="auth-card">
      ...
      document.querySelectorAll(".key").forEach(b=>b.onclick=async()=>{const k=b.dataset.key;if(k==="⌫")pin=pin.slice(0,-1);else if(k==="Enter"){if(pin.length!==4){toast("Enter 4 digits.");return;}try{const result=await api("verifyAdmin",{pin});if(result && result.ok && result.token){state.token=result.token;state.role="admin";state.admin=true;state.tab="dashboard";await loadData();render();}else{toast("Invalid Master PIN.");pin="";renderAuth();}}catch(e){toast(e.message);}}else if(pin.length<4)pin+=k;renderAuth();});
      document.getElementById("back").onclick=renderLogin;
    }; renderAuth();
  }
  ...
  function bindNav(){
    ...
    const out=document.getElementById("logout"); if(out)out.onclick=()=>{state.role=null;state.staff=null;state.admin=false;state.data=null;state.token=null;state.orderCart=[];render();};
  }
  ...
  function monitorView(){return `<div class="card"><div class="toolbar"><input id="mSearch" class="input" placeholder="Search..."><select id="mType"><option value="">All</option><option>Production</option><option>Wastage</option></select></div><div class="table-wrap"><table><thead><tr><th>Timestamp</th><th>Type</th><th>Item</th><th>Qty</th><th>Unit</th><th>Reason</th><th>Logged By</th><th>Status</th></tr></thead><tbody id="monitorRows"></tbody></table></div></div>`}
  function setupMonitor(){
    const render=()=>{
      const q=(document.getElementById("mSearch").value||"").toLowerCase(), t=document.getElementById("mType").value;
      const p=state.data.production.map(x=>({...x,__type:"Production",__id:x.Timestamp})), w=state.data.wastage.map(x=>({...x,__type:"Wastage",__id:x.Timestamp}));
      const rows=[...p,...w].filter(x=>(!q||Object.values(x).join(" ").toLowerCase().includes(q))&&(!t||x.__type===t));
      document.getElementById("monitorRows").innerHTML=rows.map(x=>`<tr><td>${esc(x.Timestamp)}</td><td>${esc(x.__type)}</td><td>${esc(x["Item Name"])}</td><td>${esc(x.Quantity)}</td><td>${esc(x.Unit)}</td><td>${esc(x.Reason||"")}</td><td>${esc(x["Logged By"])}</td><td><select class="monitor-status" data-type="${esc(x.__type)}" data-id="${esc(x.__id)}"><option ${x.Status==="Logged"?"selected":""}>Logged</option><option ${x.Status==="Reviewed"?"selected":""}>Reviewed</option><option ${x.Status==="Flagged"?"selected":""}>Flagged</option></select></td></tr>`).join("") || `<tr><td colspan="8" class="empty">No records.</td></tr>`;
      document.querySelectorAll(".monitor-status").forEach(e=>e.onchange=async()=>{try{await api("updateLog",{type:e.dataset.type,id:e.dataset.id,status:e.value});const arr=e.dataset.type==="Production"?state.data.production:state.data.wastage;const row=arr.find(r=>String(r.Timestamp)===String(e.dataset.id));if(row)row.Status=e.value;toast("Status updated.");}catch(err){toast(err.message);render();}});
    };
    document.getElementById("mSearch").oninput=render; document.getElementById("mType").onchange=render; render();
  }
  ...
  function setupSettings(){document.getElementById("saveSettings").onclick=async()=>{const url=document.getElementById("setUrl").value.trim(),email=document.getElementById("setEmail").value.trim(),pin=document.getElementById("setPin").value.trim();try{if(url) state.apiUrl=url;await api("saveSettings",{settings:{OwnerEmail:email,MasterPin:pin,InventoryCountEnabled:state.data.settings.InventoryCountEnabled}});state.data.settings.OwnerEmail=email;state.data.settings.MasterPin=pin;toast("Settings saved.");}catch(e){toast(e.message)}}}
