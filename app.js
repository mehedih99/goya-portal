...
(() => {
  ...
  function showPage(id){
    document.querySelectorAll('.app [data-page]').forEach(x=>x.classList.add('hidden'));
  ...
  function renderStaffAccess(){
    ...
    const map={order:'navOrder',production:'navProduction',wastage:'navWastage'};
    Object.entries(map).forEach(([k,id])=>$(id)?.classList.toggle('hidden',!a[k]));
    $('navInventory')?.classList.toggle('hidden',!(a.inventory || (state.data?.inventory_sessions||[]).length));
  }
  ...
  function pickItem(kind,sku){
    ...
    box.innerHTML=`<div class="card" style="box-shadow:none"><div class="section-title"><div><strong>${esc(item.name)}</strong><div class="small muted">${esc(item.sku)}</div></div><span class="tag">${esc(item.items_type)}</span></div>
      <div class="form-grid">
        ...
        ${kind==='wastage'?`<div class="field"><label>Reason / Note</label><input id="wasteReason" type="text" placeholder="Write the wastage reason…"></div>`:''}
  ...
  function addLine(kind){
    ...
    if(kind==='wastage'){x.reason=$('wasteReason').value.trim();if(!x.reason){toast('Write a wastage reason.',true);return}}
  ...
  function pickOrderItem(sku){
    ...
    <div class="form-grid">
      ...
      <div class="field"><label>Item note (optional)</label><input id="orderItemNote" placeholder="Optional note for this item…"></div>
  ...
  function addOrderLine(){
    ...
    state.cart.push({name:item.name,sku:item.sku,quantity:qty,unit,supplier:item.supplier||'',notes:$('orderItemNote')?.value.trim()||''});
    $('orderSelected').classList.add('hidden');window._selectedOrder=null;renderOrderCart();
  }
  function renderOrderCart(){
    const body=$('orderCartBody');
    body.innerHTML=state.cart.length?state.cart.map((x,i)=>`<tr><td>${esc(x.name)}<div class="small muted">${esc(x.sku)}</div></td><td>${x.quantity}</td><td>${esc(x.unit)}</td><td>${esc(x.supplier||'')}</td><td>${esc(x.notes||'')}</td><td><button class="btn danger" style="padding:6px 9px" onclick="GoyaStaff.removeOrder(${i})">Remove</button></td></tr>`).join(''):`<tr><td colspan="6" class="empty">Your cart is empty.</td></tr>`;
    $('submitOrder').disabled=!state.cart.length;
  }
  function removeOrder(i){state.cart.splice(i,1);renderOrderCart()}
  async function submitOrder(){
    try{
      const submittedItems=state.cart.map(x=>({...x}));
      const r=await rpc('submit_order',{p_token:state.token,p_notes:null,p_items:submittedItems});
      const groups={};
      submittedItems.forEach(x=>{const k=x.supplier||'Unassigned';(groups[k]??=[]).push(x)});
      const dt=new Date();
      const parts=[];
      for(const [supplier,items] of Object.entries(groups)){
        parts.push(`Supplier: ${supplier}`);
        items.forEach(x=>parts.push(`- ${x.name}: ${x.quantity} ${x.unit}${x.notes?` | Note: ${x.notes}`:''}`));
        parts.push('');
      }
      const msg=`=== NEW PURCHASE ORDER ===
------------------------------
...
Date & Time: ${dt.toLocaleString('en-AE',{dateStyle:'short',timeStyle:'medium'})}

Genareted via Goya Smart Portal`;
      state.cart=[];renderOrderCart();toast('Order submitted successfully.');
  ...
  function setOrderHistoryStatus(btn,status){
    const next=state.orderHistoryStatus===status?'':status;state.orderHistoryStatus=next;document.querySelectorAll('.history-stat').forEach(x=>x.classList.toggle('active',next&&x.dataset.status===next));renderHistory();
  }
  function renderHistory(){
    const h=state.history||{};
    const active=document.querySelector('#historyTabs button.active')?.dataset.kind||'orders';
    const tools=$('orderHistoryTools');if(tools)tools.classList.toggle('hidden',active!=='orders');
    let rows=h[active==='orders'?'orders':active]||[];
    let cols,title;
    if(active==='orders'){
      title='Order History';
      const all=rows;
      const count=st=>all.filter(x=>x.status===st).length;
      if($('histPendingCount'))$('histPendingCount').textContent=count('Pending');if($('histOrderedCount'))$('histOrderedCount').textContent=count('Ordered');if($('histReceivedCount'))$('histReceivedCount').textContent=count('Received');if($('histNotReceivedCount'))$('histNotReceivedCount').textContent=count('Not Received');
      const suppliers=[...new Set(all.map(x=>x.supplier).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
      if($('histSupplier')){const current=$('histSupplier').value;$('histSupplier').innerHTML='<option value="">All suppliers</option>'+suppliers.map(x=>`<option>${esc(x)}</option>`).join('');$('histSupplier').value=suppliers.includes(current)?current:''}
      const q=($('histOrderSearch')?.value||'').trim().toLowerCase(),od=$('histOrderDate')?.value||'',rd=$('histReceivedDate')?.value||'',sup=$('histSupplier')?.value||'',st=state.orderHistoryStatus||'';
      rows=all.filter(x=>(!st||x.status===st)&&(!q||String(x.name||'').toLowerCase().includes(q)||String(x.sku||'').toLowerCase().includes(q))&&(!od||String(x.created_at||'').slice(0,10)===od)&&(!rd||String(x.received_date||'').slice(0,10)===rd)&&(!sup||x.supplier===sup));
      cols=['order_no','name','quantity','unit','supplier','status','notes','logged_by','created_at','received_date'];
    }else if(active==='production'){title='Production History';cols=['name','sku','quantity','unit','logged_by','created_at']}
    else if(active==='wastage'){title='Wastage History';cols=['name','sku','quantity','unit','reason','logged_by','created_at']}
    else{title='Inventory History';cols=['item_name','sku','section','storage_quantity','ingredients_quantity','counted_by','counted_at']}
    $('historyTitle').textContent=title;
    const dateCols=new Set(['created_at','counted_at','received_date']);
    $('historyBody').innerHTML=rows.length?rows.map(r=>`<tr>${cols.map(c=>`<td>${esc(dateCols.has(c)?fmtDate(r[c]):(r[c]??''))}</td>`).join('')}</tr>`).join(''):`<tr><td colspan="${cols.length}" class="empty">No records for this month.</td></tr>`;
    $('historyHead').innerHTML=cols.map(c=>`<th>${esc(c.replaceAll('_',' '))}</th>`).join('');
  }

  async function submitComplaint(){
    const type=$('complaintType')?.value||'Complain',details=$('complaintText')?.value.trim()||'',hide=!!$('complaintHide')?.checked;
    if(!details){toast('Write complain/feedback details.',true);return}
    try{await rpc('submit_complaint_feedback',{p_token:state.token,p_type:type,p_details:details,p_hide_identity:hide});$('complaintText').value='';$('complaintHide').checked=false;toast('Submitted to management successfully.')}catch(e){toast(e.message,true)}
  }
  function renderReasons(){/* free-text wastage reason */}
  function renderInfo(){
  ...
  function renderInventorySession(s){
    ...
    target.innerHTML=`<div class="field"><label>Section</label><select id="invsec-${s.id}">${sections.map(x=>`<option>${esc(x)}</option>`).join('')}</select></div><div class="table-wrap" style="margin-top:10px"><table class="table"><thead><tr><th>Item</th><th>SKU</th><th>Storage Qty</th><th>Ingredients Qty</th></tr></thead><tbody>${internal.filter(x=>!sections.length||sections.some(sec=>String(x.items_type||'').toLowerCase()===String(sec).toLowerCase())).map(x=>`<tr><td>${esc(x.name)}</td><td>${esc(x.sku)}</td><td><input id="is-${s.id}-${esc(x.sku)}" type="number" step="0.001" min="0"></td><td><input id="ii-${s.id}-${esc(x.sku)}" type="number" step="0.001" min="0"></td></tr>`).join('')}</tbody></table></div><div style="margin-top:10px;text-align:right"><button class="btn ok" onclick="GoyaStaff.saveInventory('${s.id}')">Save Inventory Count</button></div>`;
  }
  async function saveInventory(sessionId){
    ...
    const items=(state.data.internal_items||[]).filter(x=>!x.items_type||String(x.items_type).toLowerCase()===String(sec).toLowerCase()).map(x=>({item_name:x.name,sku:x.sku,section:sec,storage_quantity:$('is-'+sessionId+'-'+x.sku)?.value||'',ingredients_quantity:$('ii-'+sessionId+'-'+x.sku)?.value||''})).filter(x=>x.storage_quantity!==''||x.ingredients_quantity!=='');
  ...
  window.GoyaStaff={staffLogin,searchItems,pickItem,addLine,removeLine,submitProduction,submitWastage,searchMaster,pickOrderItem,addOrderLine,removeOrder,submitOrder,staffHistory,renderHistory,setOrderHistoryStatus,saveInventory,submitComplaint,staffLogout,mobileMenu};
  ...
  async function adminLogin(){
    ...
    try{
      await initClient();const d=await rpc('admin_login',{p_pin:pin});state.token=d.token;state.admin=true;
  ...
  function renderAdminAll(){
    const d=state.adminData||{};renderMasterTable();renderInternalTable();renderSuppliers();renderStaffTable();renderReasonsAdmin();renderSettings();renderInfoAdmin();renderInventorySessions();renderComplaints();
  ...
  function renderReasonsAdmin(){
    const body=$('reasonBody');if(!body)return;const rows=state.adminData.reasons||[];body.innerHTML=rows.map(x=>`<tr><td>${esc(x.name)}</td><td>${x.active?'Active':'Disabled'}</td><td><button class="btn danger" style="padding:6px 9px" onclick="GoyaAdmin.deleteReason('${x.id}')">Delete</button></td></tr>`).join('')||'<tr><td colspan="3" class="empty">No reasons.</td></tr>';
  }
  ...
  async function loadComplaints(){try{state.adminData.complaints=await rpc('admin_complaints',{p_token:state.token});renderComplaints()}catch(e){toast(e.message,true)}}
  function renderComplaints(){
    const body=$('complaintAdminBody');if(!body)return;const type=$('complaintAdminType')?.value||'',q=($('complaintAdminSearch')?.value||'').trim().toLowerCase();
    const rows=(state.adminData?.complaints||[]).filter(x=>(!type||x.type===type)&&(!q||String(x.details||'').toLowerCase().includes(q)||String(x.staff_name||'').toLowerCase().includes(q)));
    body.innerHTML=rows.length?rows.map(x=>`<tr><td><span class="tag">${esc(x.type)}</span></td><td class="wrap-cell">${esc(x.details).replace(/\n/g,'<br>')}</td><td>${esc(x.staff_name)}</td><td>${x.hide_identity?'Yes':'No'}</td><td>${fmtDate(x.created_at)}</td></tr>`).join(''):'<tr><td colspan="5" class="empty">No complain/feedback submissions.</td></tr>';
  }
  ...
  function whatsappSupplier(){
    const rows=state.adminOrders||[];if(!rows.length){toast('No filtered orders.',true);return}
    const groups={};rows.forEach(x=>{const k=x.supplier||'Unassigned';(groups[k]??=[]).push(x)});
    const parts=[];for(const [supplier,items] of Object.entries(groups)){parts.push(`Supplier: ${supplier}`);items.forEach(x=>parts.push(`- ${x.name}: ${x.quantity} ${x.unit}${x.notes?` | Note: ${x.notes}`:''}`));parts.push('')}
    const dt=new Date(),who=rows[0]?.logged_by||'Goya Staff';
    const msg=`=== NEW PURCHASE ORDER ===
------------------------------
...
Genareted via Goya Smart Portal`;
  ...
  window.GoyaAdmin={adminLogin,adminNav,openMobileAdmin,loadAdminOrders,bulkStatus,deleteSelectedOrders,exportOrders,whatsappSupplier,loadProductionWastage,exportPW,loadInventoryCounts,exportInventory,deleteItem,editSupplier,saveSupplier,editStaff,saveStaff,deleteStaff,saveReason,deleteReason,saveSettings,editInfo,saveInfo,deleteInfo,editInventory,saveInventorySession,deleteInventory,loadComplaints,renderComplaints,handleUpload,downloadTemplate,uploadChange,exportMasterFiltered,exportInternalAll,adminLogout};

  // ---------- generic startup ----------
  document.addEventListener('DOMContentLoaded',()=>{
    localStorage.removeItem('goya_staff_session');localStorage.removeItem('goya_admin_session');
    state.token=null;state.user=null;state.admin=false;
    if(document.body.dataset.page==='staff'){state.prodCart=[];state.wasteCart=[];$('staffLogin')?.classList.remove('hidden');$('staffApp')?.classList.add('hidden')}
    if(document.body.dataset.page==='admin'){$('adminLogin')?.classList.remove('hidden');$('adminApp')?.classList.add('hidden')}
  });
})();
