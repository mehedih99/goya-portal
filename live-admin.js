(() => {
  ...
  function matchedShift(row){
    ...
    shifts.forEach(v=>{const [h,m]=String(v).split(':').map(Number),sm=h*60+m,dist=Math.abs(punch-sm);if(bestDist==null||dist<bestDist||(dist===bestDist&&sm>best)){best=sm;bestDist=dist}});
    return best;
  }
  ...
  function effectiveStartMs(row,session,index){
    ...
    if(index===0){const sm=matchedShift(row);if(sm!=null&&row.operational_date){const h=Math.floor(sm/60),m=sm%60,sch=new Date(`${row.operational_date}T${pad(h)}:${pad(m)}:00+04:00`).getTime();if(Number.isFinite(sch)&&st<sch)st=sch}}
    return st;
  }
  ...
  async function editDay(staffId,date){
    ...
    const dayBox=$('liveEditDayControls');if(dayBox)dayBox.innerHTML=`<div class="live-edit-day-grid"><label>Break Taken<select id="liveEditBreak"><option value="true"${r.break_taken!==false?' selected':''}>Yes</option><option value="false"${r.break_taken===false?' selected':''}>No</option></select></label><label>Status<select id="liveEditClassification">${classificationOptions(r.classification||'Duty')}</select></label><label class="live-edit-note">Note (optional)<input id="liveEditNote" value="${esc(r.classification_note||'')}" placeholder="Optional admin note"></label><button class="btn success" onclick="GoyaLiveAdmin.saveDayEdit()">Save Day Settings</button></div>`;
  ...
  async function saveDayEdit(){if(!editTarget)return;try{await rpc('attendance_live_admin_update_day',{p_token:ctx().token,p_staff_id:editTarget.staffId,p_date:editTarget.date,p_break_taken:$('liveEditBreak').value==='true',p_classification:$('liveEditClassification').value,p_note:$('liveEditNote').value.trim()||null});toast('Day settings saved and recalculated.');await refreshVisible();await editDay(editTarget.staffId,editTarget.date)}catch(e){toast(e.message,true)}}
