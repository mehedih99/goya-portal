(() => {
  ...
  function exportOvertimePDF(){
    const data=overtimeSummaryRows();if(!data.length)return toast('No overtime found for the selected period.',true);if(!window.jspdf?.jsPDF)return toast('PDF library is still loading.',true);
    const {jsPDF}=window.jspdf,doc=new jsPDF({orientation:'portrait'}),from=$('liveHistFrom').value,to=$('liveHistTo').value;
    const total=data.reduce((a,x)=>a+x.ot,0),totalDays=data.reduce((a,x)=>a+x.days,0),highest=data.slice().sort((a,b)=>b.ot-a.ot)[0],staffSelected=$('liveHistStaff')?.value;
    doc.setFontSize(19);doc.text('GOYA - Overtime Only Report',14,16);
    doc.setFontSize(9);doc.setTextColor(92);doc.text(`Period: ${from} to ${to}`,14,24);doc.setTextColor(0);
    const cards=staffSelected
      ? [['OT Days',totalDays],['Total OT',otText(total)],['Average / OT Day',totalDays?otText(total/totalDays):'0']]
      : [['Staff with OT',data.length],['Total OT Days',totalDays],['Total OT',otText(total)],['Highest OT Staff',highest?.staff||'—'],['Highest OT',highest?otText(highest.ot):'0']];
    let x=14,y=30;cards.forEach(([k,v],i)=>{const w=staffSelected?56:35.5;doc.setFillColor(246,249,247);doc.setDrawColor(216,227,221);doc.roundedRect(x,y,w,19,3,3,'FD');doc.setFontSize(7.2);doc.setTextColor(92,108,100);doc.text(String(k),x+3,y+6);doc.setFontSize(10.5);doc.setTextColor(28,54,45);const val=String(v);doc.text(val.length>18?val.slice(0,18):val,x+3,y+14);x+=w+4});
    doc.setTextColor(0);
    doc.autoTable({startY:55,head:[['Staff','OT Days','Total OT']],body:data.map(x=>[x.staff,x.days,otText(x.ot)]),styles:{fontSize:9,cellPadding:3,textColor:[42,56,49]},headStyles:{fillColor:[48,81,70],textColor:[255,255,255]},alternateRowStyles:{fillColor:[248,250,249]},margin:{left:14,right:14}});
  ...
  function closeExportMenuOnOutsideClick(){
    document.addEventListener('click',e=>{
      document.querySelectorAll('.live-export-menu[open]').forEach(menu=>{
